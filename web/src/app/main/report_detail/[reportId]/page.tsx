...
<body style="padding:32px;max-width:800px;margin:0 auto;">
  ...
  <div style="display:flex;gap:12px;margin-bottom:28px;">
    <div style="flex:1;background:#f2f3f5;border-radius:12px;padding:16px;border:1px solid #d9dbe0;">
      <div style="font-size:13px;font-weight:700;color:#141a24;margin-bottom:10px;">강점 분야</div>
      ${report.strengthAreas.map((s) => `<div style="font-size:13px;color:#141a24;margin-bottom:4px;">✓ ${s}</div>`).join('')}
    </div>
    <div style="flex:1;background:#f2f3f5;border-radius:12px;padding:16px;border:1px solid #d9dbe0;">
      <div style="font-size:13px;font-weight:700;color:#141a24;margin-bottom:10px;">발전 권장 영역</div>
...
export default function ReportDetailScreen() {
  ...
  return (
    <div className="flex flex-1 flex-col bg-paper pb-10" data-report-id={reportId}>
      {/* ── 상단 네비 ── */}
      <div
        className="no-print flex items-center justify-between border-b border-line bg-card px-5 pb-3"
        style={{ paddingTop: "calc(var(--sat) + 12px)" }}
      >
        <button type="button" onClick={goBack} className="tap">
          <span className="text-[16px] font-medium text-gold">← 목록</span>
        </button>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="tap rounded-lg border border-line px-[14px] py-2"
          >
            <span className="text-[15px] font-medium text-fg2">인쇄</span>
          </button>
          <button
            type="button"
            onClick={() => void handleDownloadPdf()}
            disabled={pdfLoading}
            className={`tap flex items-center justify-center rounded-lg border border-gold bg-brand px-[14px] py-2 ${
          ...
          >
            {pdfLoading ? (
              ...
              <span className="text-[15px] font-bold text-ink">PDF 저장·공유</span>
            )}
          </button>
        </div>
      </div>
      ...
      {/* ── 종합 헤더 카드 ── */}
      <div className="mx-5 mt-4 rounded-2xl border border-line bg-card p-[18px]">
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-3xl bg-elev">
            <span className="text-[22px] font-bold text-gold">{report.studentName.charAt(0)}</span>
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[18px] font-bold text-fg">{report.studentName} 학생</p>
        ...
        <div className="mb-[6px] flex justify-between">
          <span className="text-[15px] font-medium text-sub">종합 점수</span>
        ...
        {/* 성향 유형 */}
        <div className="rounded-[10px] border border-line bg-paper p-[14px]">
          <p className="mb-[6px] text-[16px] font-bold text-fg">{report.personalityType}</p>
          <p className="text-[15px] leading-[22px] text-fg2">{report.personalityDesc}</p>
        </div>
      </div>

      {/* ── 강점 / 발전 영역 ── */}
      <div className="mx-5 mt-3 flex gap-[10px]">
        <div className="min-w-0 flex-1 rounded-[14px] border border-line bg-elev p-[14px]">
          <p className="mb-[10px] text-[14px] font-bold text-fg">강점 분야</p>
          {report.strengthAreas.map((s, i) => (
            <div key={i} className="mb-[6px] flex items-center gap-[6px]">
              <span className="h-[6px] w-[6px] shrink-0 rounded-[3px] bg-gold" />
              <span className="flex-1 text-[14px] text-gold">{s}</span>
            </div>
          ))}
        </div>
        <div className="min-w-0 flex-1 rounded-[14px] border border-line bg-elev p-[14px]">
          <p className="mb-[10px] text-[14px] font-bold text-fg">발전 권장</p>
      ...
      {/* ── 담임 총평 ── */}
      <div className="mx-5 mt-3 rounded-[14px] border border-line bg-card p-4">
        <p className="mb-[10px] text-[14px] font-bold uppercase tracking-[0.5px] text-sub">
          담임 강사 종합 총평
        </p>
        <p className="text-[16px] leading-[25px] text-fg2">{report.overallComment}</p>
      </div>

      {/* ── 프로그램별 상세 ── */}
      <h2 className="mx-5 mt-6 mb-[10px] text-[16px] font-bold text-fg">
        프로그램별 상세 평가
      </h2>
...
function ProgramCard({
  ...
  return (
    <div className="mx-5 mb-[10px] overflow-hidden rounded-[14px] border border-line bg-card">
      {/* 헤더 (항상 표시) */}
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        ...
        <span
          className={`inline-block text-[22px] ${expanded ? "rotate-90 text-gold" : "rotate-0 text-sub"}`}
        >
          ›
        </span>
      </button>
      ...
      <div className="flex items-center gap-[10px] px-[14px] pb-3">
        <ProgressBar
          ...
          className="flex-1"
        />
        <div className="shrink-0 rounded-[10px] bg-elev px-2 py-[3px]">
          <span className="text-[14px] font-bold text-gold">+{prog.growthIndex}↑ 성장</span>
        </div>
      </div>
      ...
      <Collapse open={expanded}>
        <div className="border-t border-line p-[14px]">
          ...
          {/* 강사 총평 */}
          <div className="mt-[14px] mb-[10px] rounded-[10px] bg-paper p-3">
            <p className="mb-[6px] text-[14px] font-bold text-fg2">강사 총평</p>
            <p className="text-[15px] leading-[22px] text-fg2">{prog.instructorComment}</p>
          </div>

          {/* 인상적이었던 점 */}
          {prog.highlights.length > 0 && (
            <div className="mb-[10px] rounded-[10px] border border-line bg-elev p-3">
              <p className="mb-2 text-[14px] font-bold text-fg2">인상적이었던 점</p>
              {prog.highlights.map((h, i) => (
                <div key={i} className="mb-[5px] flex gap-[6px]">
                  <span className="text-[14px] text-faint">★</span>
                  <p className="flex-1 text-[15px] leading-[21px] text-fg2">{h}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="rounded-[10px] border border-line bg-elev p-3">
            <p className="mb-2 text-[14px] font-bold text-fg">향후 발전 방향</p>
            {prog.nextSteps.map((s, i) => (
              <div key={i} className="mb-[6px] flex items-start gap-2">
                <div className="mt-px flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-[9px] bg-gold">
                  <span className="text-[14px] font-bold text-ink">{i + 1}</span>
                </div>
                <p className="flex-1 text-[15px] leading-[21px] text-fg">{s}</p>
              </div>
            ))}
          </div>
        </div>
...
function GrowthBar({ label, score, color }: { label: string; score: number; color: string }) {
  return (
    <div className="flex flex-1 flex-col items-center">
      ...
      <p className="mb-1 text-[14px] leading-[18px] text-sub">{label}</p>
      <p className="mb-1 text-[17px] font-extrabold leading-[22px]" style={{ color }}>
        {score}점
      </p>
      <div className="flex h-10 w-[70%] flex-col justify-end overflow-hidden rounded bg-elev">
  ...
  return (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-2">
        <div className="relative h-2 flex-1 overflow-visible rounded bg-elev">
          {/* 또래 평균 */}
          <div
            className="absolute -top-[2px] -bottom-[2px] z-[1] w-[2px] bg-line2"
