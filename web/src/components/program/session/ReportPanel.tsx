    ...
    <>
      ...
      {record.feedback && (
        <Card title={absent ? "선생님 안내" : "선생님 한마디"} icon="💬">
          <blockquote className="rounded-2xl border-l-4 border-brand bg-brand-light px-4 py-4">
            <p className="mb-1 text-[15px] font-bold text-brand">{record.instructorName} 강사</p>
            <p className="text-[17px] leading-[27px] text-gray-900">{record.feedback}</p>
          </blockquote>
        </Card>
      )}
        ...
        <Card title="수업 참여" icon="🙌">
          {record.participationScore != null && (
            <InfoRow label="참여도">
              ...
              <span className="text-gray-600"> · {record.participationScore}점</span>
              <ProgressBar
          ...
          {record.homeworkDone != null && (
            <InfoRow label="과제">
              {record.homeworkDone ? (
                <span className="font-bold text-green-700">✓ 제출 완료</span>
              ) : (
                <span className="font-bold text-red-600">미제출</span>
              )}
            </InfoRow>
          )}
        </Card>
      ...
      {!absent && evaluation && (
        <Card title="이 수업 평가" icon="🏅">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[15px] text-gray-600">종합 점수</p>
              <p className="text-[24px] font-extrabold leading-[30px] text-gray-900">
                {evaluation.overallScore}
                <span className="text-[16px] font-semibold text-gray-600">점</span>
              </p>
            </div>
          </div>
            ...
            {evaluation.competencies.map((c) => (
              <li key={c.label}>
                <div className="mb-1 flex items-baseline justify-between gap-2">
                  <span className="text-[16px] font-semibold text-gray-800">{c.label}</span>
                  <span className="shrink-0 text-[16px] font-bold text-gray-900">
                    {c.score}
                    <span className="text-[14px] font-medium text-gray-500"> / 또래 {c.benchmark}</span>
                  </span>
                </div>
          ...
          {onOpenFullReport && (
            <button
              ...
              className="tap mt-5 w-full rounded-2xl border border-gray-200 py-4 text-center text-[16px] font-bold text-brand"
            >
              📊 종합 리포트 보기
            </button>
          )}
