...
- 주소를 열면 **등록코드·전화 인증 없이 바로 메인 화면**. 보이는 계정은 **010-7656-7933** (보호자 · 달성캠퍼스, 자녀 김민준 · 이서연 — 운영 DB 값을 2026-09-17 에 옮겨 둔 고정값, `src/lib/demo.ts`)
...
`e2e/README.md` 참고. 실제 Cloud Functions 코드를 로컬 에뮬레이터에서 돌리고, 헤드리스 브라우저로 **등록코드 → 생년월일 → OTP → 홈 → 프로그램/리포트/공유/인쇄 → 알림 → 내정보(보호자 초대) → 로그아웃 → 딥링크 가드 → 재로그인 → 자녀 2명 전환 → 회차 목록·회차 화면(출결·일정·내용·Q&A·리포트 탭, 이전/다음 회차) → 수강 예정 요약·안내 화면(목적·회차별 내용·공지·규정·Q&A, 홈에서는 숨김) → 홈 규정·지침 버튼·이전 수강 이력 → 초대받은 보호자 로그인 → 회원 탈퇴(서버 정리 확인·재로그인 시 자동 연결 안 됨)** 까지 55개 시나리오를 검증한다.
